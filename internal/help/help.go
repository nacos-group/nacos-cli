...
var (
	...
	ConfigSet = CommandHelp{
		...
		Parameters: []string{
			...
			"--type, -t      Config format: yaml | json | xml | html | properties | toml | text",
			"                Auto-detected from --file or dataId extension if omitted",
		},
		Examples: []string{
			"# Publish from file (type auto-detected as yaml from extension)",
			"config-set application.yaml DEFAULT_GROUP --file ./application.yaml",
			"",
			"# Explicitly specify type",
			"config-set app-config DEFAULT_GROUP -f ./app.yaml --type yaml",
			"",
			"# Publish from stdin",
